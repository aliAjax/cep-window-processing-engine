...
func New(s *domain.Store, e *runtime.Engine, c *checkpoint.Manager) *Server {
	coordinator := checkpoint.NewCoordinator(c, checkpoint.NewRegistry(), repository.NewCheckpointJobs(), observability.NewCheckpointMetrics())
	return &Server{Store: s, Engine: e, Checkpoint: c, Coordinator: coordinator}
}
...
func (s *Server) matches(w http.ResponseWriter, r *http.Request) { write(w, s.Store.Matches) }
...
func (s *Server) save(w http.ResponseWriter, r *http.Request) {
	ids, e := s.Coordinator.SaveBatch(r.Context(), "default", []*domain.Store{s.Store})
	if e != nil {
		http.Error(w, e.Error(), 500)
		return
	}
	write(w, map[string]string{"checkpoint_id": ids[0]})
}
